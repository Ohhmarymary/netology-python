...
# каждого предмета не обязательно были все типы занятий. Сформировать словарь, содержащий название предмета и общее количество
...
vac = {}
with open("01.txt", 'r') as file_obj:
    for text in file_obj.readlines():
        line = text.replace('(', ' ').split()
        vac[line[0][:-1]] = sum(
            int(i) for i in line if i.isdigit()
        )
